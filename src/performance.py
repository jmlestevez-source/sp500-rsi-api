# src/performance.py
"""
Tracking de rendimiento del portfolio vs SPY.
Registra todas las operaciones y calcula métricas reales.
"""

import json
import numpy as np
import yfinance as yf
import pandas as pd
from pathlib import Path
from datetime import datetime


HISTORY_PATH    = Path("data/performance/history.json")
TRADE_LOG_PATH  = Path("data/trades/trade_log.json")


# ── Trade Log ─────────────────────────────────────────────────────────────────

def load_trade_log() -> list[dict]:
    if TRADE_LOG_PATH.exists():
        return json.load(open(TRADE_LOG_PATH))
    return []


def save_trade_log(trades: list[dict]) -> None:
    TRADE_LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
    with open(TRADE_LOG_PATH, "w") as f:
        json.dump(trades, f, indent=2)


def record_trades(
    result: dict,
    scenarios: dict,
    current_positions: dict,
) -> list[dict]:
    """
    Registra las operaciones ejecutadas en este rebalanceo.
    Calcula P&L de posiciones cerradas.
    """
    trades     = load_trade_log()
    today      = datetime.now().strftime("%Y-%m-%d")
    new_trades = []

    # Posiciones abiertas / aumentadas
    for ticker, new_weight in result["weights"].items():
        old_weight = current_positions.get(
            ticker, {}
        ).get("weight", 0)
        scenario   = scenarios.get(ticker, {})
        price      = scenario.get("current_price", 0)

        if ticker in result["added_names"]:
            action = "OPEN"
        elif new_weight > old_weight + 0.01:
            action = "ADD"
        elif new_weight < old_weight - 0.01:
            action = "TRIM"
        else:
            action = "HOLD"

        if action in ("OPEN", "ADD", "TRIM"):
            trade = {
                "date":          today,
                "ticker":        ticker,
                "action":        action,
                "price_at_trade": price,
                "weight_before": old_weight,
                "weight_after":  new_weight,
                "ev_12m":        scenario.get("ev_12m"),
                "kill_condition": scenario.get(
                    "kill_condition"
                ),
                "status":        "OPEN",
                "pnl_pct":       None,
                "pnl_days":      None,
            }
            new_trades.append(trade)

    # Posiciones cerradas → calcular P&L
    for ticker in result["dropped_names"]:
        old_pos  = current_positions.get(ticker, {})
        entry_p  = old_pos.get("entry_price") or 0
        scenario = scenarios.get(ticker, {})
        exit_p   = scenario.get("current_price", 0)
        entry_d  = old_pos.get("entry_date", today)

        pnl_pct  = (
            (exit_p - entry_p) / entry_p * 100
            if entry_p > 0 else None
        )

        try:
            d0    = datetime.strptime(entry_d, "%Y-%m-%d")
            d1    = datetime.strptime(today,   "%Y-%m-%d")
            days  = (d1 - d0).days
        except Exception:
            days = None

        trade = {
            "date":           today,
            "ticker":         ticker,
            "action":         "CLOSE",
            "price_at_trade": exit_p,
            "weight_before":  old_pos.get("weight", 0),
            "weight_after":   0,
            "entry_price":    entry_p,
            "entry_date":     entry_d,
            "exit_price":     exit_p,
            "pnl_pct":        pnl_pct,
            "pnl_days":       days,
            "status":         "CLOSED",
        }
        new_trades.append(trade)

    # Actualizar log existente
    # Marcar como CLOSED los que se cerraron
    closed_tickers = set(result["dropped_names"])
    for t in trades:
        if (
            t["ticker"] in closed_tickers
            and t["status"] == "OPEN"
        ):
            t["status"] = "CLOSED"

    trades.extend(new_trades)
    save_trade_log(trades)
    return new_trades


# ── Performance History ───────────────────────────────────────────────────────

def load_performance_history() -> dict:
    if HISTORY_PATH.exists():
        return json.load(open(HISTORY_PATH))
    return {
        "inception_date":   None,
        "initial_value":    100_000,
        "snapshots":        [],
        "spy_snapshots":    [],
    }


def save_performance_history(h: dict) -> None:
    HISTORY_PATH.parent.mkdir(parents=True, exist_ok=True)
    with open(HISTORY_PATH, "w") as f:
        json.dump(h, f, indent=2)


def update_performance(
    result: dict,
    positions: dict,
    scenarios: dict,
) -> dict:
    """
    Actualiza el histórico de rendimiento.
    Calcula valor de portfolio basado en precios reales.
    """
    history = load_performance_history()
    today   = datetime.now().strftime("%Y-%m-%d")

    # Inicializar si es la primera vez
    if not history["inception_date"]:
        history["inception_date"] = today

    # Valor actual del portfolio (pesos × precios reales)
    portfolio_value = 0.0
    position_values = {}

    for ticker, weight in result["weights"].items():
        scenario = scenarios.get(ticker, {})
        price    = scenario.get("current_price", 0)
        old_pos  = positions.get(ticker, {})

        # Calcular rendimiento desde entrada
        entry_price = old_pos.get("entry_price") or price
        entry_date  = old_pos.get("entry_date",  today)

        ret_since_entry = (
            (price - entry_price) / entry_price
            if entry_price > 0 else 0
        )

        position_values[ticker] = {
            "weight":            weight,
            "current_price":     price,
            "entry_price":       entry_price,
            "entry_date":        entry_date,
            "ret_since_entry_pct": ret_since_entry * 100,
        }

    # Obtener precio SPY real
    spy_price = None
    try:
        spy_hist  = yf.Ticker("^GSPC").history(period="2d")
        spy_price = float(spy_hist["Close"].iloc[-1])
    except Exception:
        pass

    snapshot = {
        "date":           today,
        "weights":        result["weights"],
        "position_values": position_values,
        "expected_return": result["expected_return"],
        "spy_price":       spy_price,
        "n_positions":     len(result["weights"]),
        "turnover":        result["turnover_used"],
    }

    history["snapshots"].append(snapshot)
    save_performance_history(history)
    return history


def compute_performance_metrics(
    positions: dict,
    scenarios: dict,
) -> dict:
    """
    Calcula métricas de rendimiento reales
    usando precios de entrada y precios actuales.
    """
    metrics = {
        "positions_detail":    [],
        "portfolio_return_pct": 0.0,
        "spy_return_pct":       0.0,
        "alpha_pct":            0.0,
        "best_position":        None,
        "worst_position":       None,
        "closed_trades_summary": {},
    }

    if not positions:
        return metrics

    # Rendimiento por posición
    total_weighted_return = 0.0
    total_weight          = 0.0
    position_returns      = []

    for ticker, pos in positions.items():
        entry_price = pos.get("entry_price") or 0
        entry_date  = pos.get("entry_date",  "")
        weight      = pos.get("weight",       0)
        scenario    = scenarios.get(ticker,   {})
        curr_price  = scenario.get("current_price", 0)

        # Si no tenemos precio actual, usar yfinance
        if not curr_price and entry_price:
            try:
                hist = yf.Ticker(ticker).history(period="2d")
                if not hist.empty:
                    curr_price = float(
                        hist["Close"].iloc[-1]
                    )
            except Exception:
                curr_price = entry_price

        ret_pct = (
            (curr_price - entry_price) / entry_price * 100
            if entry_price > 0 else 0
        )

        # Calcular días en posición
        try:
            d0   = datetime.strptime(entry_date, "%Y-%m-%d")
            days = (datetime.now() - d0).days
        except Exception:
            days = 0

        pos_detail = {
            "ticker":       ticker,
            "weight":       weight,
            "entry_price":  entry_price,
            "entry_date":   entry_date,
            "current_price": curr_price,
            "ret_pct":      ret_pct,
            "days_held":    days,
            "ev_12m":       pos.get("ev_12m"),
        }
        position_returns.append(pos_detail)

        total_weighted_return += ret_pct * weight
        total_weight          += weight

    metrics["positions_detail"] = sorted(
        position_returns, key=lambda x: x["ret_pct"],
        reverse=True
    )

    if total_weight > 0:
        metrics["portfolio_return_pct"] = (
            total_weighted_return / total_weight
        )

    # Mejor y peor posición
    if position_returns:
        metrics["best_position"]  = max(
            position_returns, key=lambda x: x["ret_pct"]
        )
        metrics["worst_position"] = min(
            position_returns, key=lambda x: x["ret_pct"]
        )

    # Rendimiento SPY en el mismo periodo
    # Usamos la fecha de inicio más antigua del portfolio
    if positions:
        dates = []
        for pos in positions.values():
            ed = pos.get("entry_date", "")
            if ed:
                try:
                    dates.append(
                        datetime.strptime(ed, "%Y-%m-%d")
                    )
                except Exception:
                    pass

        if dates:
            oldest_date = min(dates)
            try:
                spy_hist = yf.Ticker("^GSPC").history(
                    start=oldest_date.strftime("%Y-%m-%d"),
                    end=datetime.now().strftime("%Y-%m-%d"),
                )
                if len(spy_hist) >= 2:
                    spy_ret = (
                        float(spy_hist["Close"].iloc[-1])
                        / float(spy_hist["Close"].iloc[0])
                        - 1
                    ) * 100
                    metrics["spy_return_pct"] = spy_ret
                    metrics["alpha_pct"] = (
                        metrics["portfolio_return_pct"]
                        - spy_ret
                    )
            except Exception:
                pass

    # Resumen de operaciones cerradas
    trades = load_trade_log()
    closed = [
        t for t in trades
        if t.get("status") == "CLOSED"
        and t.get("pnl_pct") is not None
    ]
    if closed:
        pnls = [t["pnl_pct"] for t in closed]
        metrics["closed_trades_summary"] = {
            "count":        len(closed),
            "avg_pnl_pct":  round(sum(pnls) / len(pnls), 2),
            "win_rate_pct": round(
                sum(1 for p in pnls if p > 0)
                / len(pnls) * 100, 1
            ),
            "best_trade":   round(max(pnls), 2),
            "worst_trade":  round(min(pnls), 2),
            "total_closed": len(closed),
        }

    return metrics
